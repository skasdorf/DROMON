...
public:
  ...
  void print_matrix();
  std::vector<CoefficientType> multiply(std::vector<CoefficientType> solution);
  std::vector<CoefficientType> get_matrix_copy();
...
template <class CoefficientType, class MatrixSolvingPolicy>
void Matrix<CoefficientType, MatrixSolvingPolicy>::print_matrix() {
  assert(is_safe && "The system has been solved, therefore the RHS cannot be returned!");
  //return *RHS_data;
...
template <class CoefficientType, class MatrixSolvingPolicy>
std::vector<CoefficientType>
Matrix<CoefficientType, MatrixSolvingPolicy>::multiply(std::vector<CoefficientType> solution) {
  assert(is_safe && "The system has been solved, therefore the RHS cannot be returned!");
  //return *RHS_data;
  std::vector<CoefficientType> out(this->m);
  ...
  for (unsigned int i = 0; i < this->m; ++i)
  {
    ...
    {
      out[i] += LHS[this->n*i+j] * solution[j];
      //out[i] += LHS_data.operator()(i,j) * solution[j];
      // out[i] += LHS_data.at(i,j) * solution[j];
      // std::cout << LHS_data[this->n*i+j] << "  ";
...
template <class CoefficientType, class MatrixSolvingPolicy>
std::vector<CoefficientType>
Matrix<CoefficientType, MatrixSolvingPolicy>::get_matrix_copy() {
  assert(is_safe && "The system has been solved, therefore the RHS cannot be returned!");
  //return *RHS_data;
    ...
    RHS_from_galerkin_system(GalerkinSystem *galerkinSystem, typename GalerkinSystem::DoFHandlerType* dof_handler)
{
  ...
  for (unsigned int i = 0; i < n_cells; ++i) {
    ...
    const auto& subexcitation_test = galerkinSystem->subexcitation_at(i);
    //std::cout << "n_cells RHS: " << n_cells << std::endl;
    // Loop through all the DoFs on this given cell
    const auto& dof_test_parent = dof_handler->get_dof_parent(i);
    ...
    for (unsigned int dof_test_index = 0;
      ...
      //std::cout << "n_dofs: " << dof_test_parent.n_dofs() << std::endl;
      //std::cout << "active_test_index: " << active_test_index << std::endl;
      //std::cout << "global_test_index: " << global_test_index << std::endl;
      //      #ifndef DEBUG
      //      #pragma omp critical
      //      #endif
      this->operator()(active_test_index) +=
      ...
      //std::cout << "subexcitation_test: " << subexcitation_test.const_at(global_test_index) << std::endl;
    }
  }
}
