...
namespace kernels
{
   ...
   */
  template <unsigned int spacedim, class Real = double, class MatValueType> //spacedim:integer specifying the space dimensions, Real: scaler numeric type, MatValueType: Material Parameter contains $$\epsilon$$ or $$\mu$$
  Point<spacedim, std::complex<Real>> grad_g(
    ...
    const MatValueType& mur)
  {
    std::complex<Real> gamma = std::complex<Real>(0.,1.)*omega*sqrt(epsr*mur)*constants<Real>::ROOT_EPS0MU0_;//Defines the complex propagation constant $$\gamma=i\omega \sqrt{\epsilon_r\mu_r}\sqrt{\epsilon_0\mu_0}$$
